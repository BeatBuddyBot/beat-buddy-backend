...
from src.player.schemas import PlayerStartSong, PlayerStartPlaylist
...
def start_playlist(playlist_data: PlayerStartPlaylist, session: Session = Depends(get_session)):
    query = select(Song.url).where(Song.playlist_id == playlist_data.playlist_id).order_by(Song.position)
